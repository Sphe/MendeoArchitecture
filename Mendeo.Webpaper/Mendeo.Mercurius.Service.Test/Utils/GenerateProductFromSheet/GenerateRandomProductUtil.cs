...
namespace Mendeo.Mercurius.Service.Test.Utils
{
    public class GenerateRandomProductUtil
    {
        /// <summary>
        /// Global Vars & Constants
        /// </summary>
        private static string debugPrefix = "[MercuriusServiceTests-GenerateRandomProductUtil]: ";
        ...
        /// </summary>
        private static bool isLoggingDebug = true;
        private static bool isLoggingVerbose = false;
        private static bool isLoggingProductFullDetailsOnCreation = false;

        public static string serviceCall = "serviceCall";
        public static string uiTestCall = "uiTestCall";
        public string callType;

        public GenerateRandomProductUtil(bool pIsLoggingDebug, bool pIsLoggingVerbose, bool pIsLoggingProductFullDetailsOnCreation, string pCallType)
        {
            isLoggingDebug = pIsLoggingDebug;
            isLoggingVerbose = pIsLoggingVerbose;
            isLoggingProductFullDetailsOnCreation = pIsLoggingProductFullDetailsOnCreation;

            // instantiate
            Debug.WriteLine(debugPrefix + "Starting Excel application");
            excelApp = new Microsoft.Office.Interop.Excel.Application();
            excelApp.Visible = true;
            if (pCallType.Equals(serviceCall))
            {
                callType = "S";
            }
            else if (pCallType.Equals(uiTestCall))
            {
                callType = "UI";
            }
            else
        ...
        public void CloseExcelApi()
        {
            excelApp.Quit();
        }

        public String GetCurrentGeneratedCategoryName()
        {
        ...
        #region All private methods Randomizers
        ...
        #region test Code random product
        private void testGenerationProduct()
        {
            ...
            for (int i = 0; i < 65; i++)
            {
                ...
                for (int v = 0; v < numberOfAttributeItemNamesPerCategory; v++)
                {
                    ...
                    Debug.WriteLine(debugPrefix + string.Concat("This is a random Category attribute type [ID=", categoryCurrentID, "|CategoryName=", categoryCurrentName, "|ProductNumber=", v, "|AttributeTypeName=", attTypeCurrentValue, "|AttributeItemName(", attItemCurrentCol, ")=", attItemCurrentValue, "]"));
