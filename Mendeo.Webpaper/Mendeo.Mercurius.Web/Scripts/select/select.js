            ...
            function($scope, $element, $timeout, $filter, RepeatParser, uiSelectMinErr, uiSelectConfig) {
                ...
                ctrl.activate = function(initSearchValue, avoidReset) {
                    if (!ctrl.disabled  && !ctrl.open) {
                        ...
                        ctrl.searchBox && ctrl.searchBox.css('left', rc.left)
                            .css('top', rc.bottom)
                            .css('width', rc.width)
                            .css('display', 'block');
                        ctrl.uiSelectLayer && ctrl.uiSelectLayer.css('display', 'block');
                ...
                ctrl.close = function(skipFocusser) {
                     ...
                     * ER_ID 1 Starts
                     */
                        ctrl.searchBox && ctrl.searchBox.css('display', 'none');
                    ctrl.uiSelectLayer && ctrl.uiSelectLayer.css('display', 'none');
                    /**
                     * ER_ID 1 Ends
                     */
                };
...
//            contains = window.jQuery.contains(element[0], e.target);
                                ...
                                contains = window.jQuery.contains(element[0], e.target) || window.jQuery.contains($select.uiSelectLayer[0], e.target);
...
//            contains = element[0].contains(e.target);
                                ...
                                contains = element[0].contains(e.target) || $select.uiSelectLayer[0].contains(e.target);
                        ...
                        scope.$on('$destroy', function() {
                            ...
                            searchBox.remove();
                            uiSelectLayer.css('display', 'none');
                        ...
                        /**
                         * ER_ID          ::      1
                         * Description    ::      Choices dropdown should append in body. So It not affect the height of original control. There should be one layer
                         *                        so that after dropdown opened, mouse scroll should not work;
                         * Solution       ::      To create a full screen size layer in document.body, and toggle its visibility on dropdown visibility.
                         *                        Find dropdown DOM and move it to document.body.
                         * Author         ::      Dhaval Patel (9th November, 2014)
                         * Future Note    ::      If in future angular-ui-select team solves this problem, remove this check and remove this file from vendor and move it to bower components.
                         * Last checked   ::      0.9.3
                         * ER_ID 1 Starts
                         */

                        scope.$select.uiSelectLayer

                        var body = $(document.body);
                        var uiSelectLayer =  $select.uiSelectLayer = body.find('.ui-select-searchbox-layer');

                        if (!uiSelectLayer || !uiSelectLayer.length) {
                            uiSelectLayer = $select.uiSelectLayer = $('<div class="ui-select-searchbox-layer ui-select ui-select-bootstrap">');
                            body.append(uiSelectLayer);
                    }

                        var searchBox = $select.searchBox = element.find(angular.isDefined(attrs.multiple) ? '.ui-select-choices-content' : '.select2-drop, .select2-with-searchbox');

                        if (searchBox && searchBox.length === 1) {
                            searchBox.css('position', 'absolute')
                                .css('display', 'none');

                            uiSelectLayer.append(searchBox);
                        }
...
angular.module("ui.select").run(["$templateCache", function($templateCache) {$templateCache.put("bootstrap/choices.tpl.html","<ul class=\"ui-select-choices ui-select-choices-content dropdown-menu\" role=\"menu\" aria-labelledby=\"dLabel\" ng-show=\"$select.items.length > 0\"><li class=\"ui-select-choices-group\"><div class=\"divider\" ng-show=\"$select.isGrouped && $index > 0\"></div><div ng-show=\"$select.isGrouped\" class=\"ui-select-choices-group-label dropdown-header\" ng-bind-html=\"$group.name\"></div><div class=\"ui-select-choices-row\" ng-class=\"{active: $select.isActive(this), disabled: $select.isDisabled(this)}\"><a href=\"javascript:void(0)\" class=\"ui-select-choices-row-inner\"></a></div></li></ul>");
    ...
    $templateCache.put("select2/match.tpl.html","<a tabindex='-1' class=\"select2-choice ui-select-match\" ng-class=\"{\'select2-default\': $select.isEmpty()}\" ng-click=\"$select.activate()\"><span ng-show=\"$select.isEmpty()\" class=\"select2-chosen\">{{$select.placeholder}}</span> <span ng-hide=\"$select.isEmpty()\" class=\"select2-chosen\" ng-transclude=\"\"></span> <abbr ng-if=\"$select.allowClear && !$select.isEmpty()\" class=\"select2-search-choice-close\" ng-click=\"$select.select(undefined)\"></abbr> <span class=\"select2-arrow ui-select-toggle\" ng-click=\"$select.toggle($event)\"><b></b></span></a>");
