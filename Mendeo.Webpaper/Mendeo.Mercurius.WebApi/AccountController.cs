...
using Mendeo.Mercurius.Mvc.Results;
...
namespace Mendeo.Mercurius.WebApi
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            {
                var mendeoUser = _userService.CreateUser(model.UserName, model.Email, model.Phone);

                if (mendeoUser.Errors.Count > 0)
                {
                    return GetErrorResultFromTuple(mendeoUser.Errors);
                }

                var user = new ApplicationUser() { UserName = model.UserName, Email = model.Email, UserId = mendeoUser.Entity.UserID };
        ...
        public async Task<IHttpActionResult> RegisterExternal(RegisterExternalBindingModel model)
        {
                     ...
                     new TransactionOptions { IsolationLevel = System.Transactions.IsolationLevel.ReadUncommitted }, TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                var user = new ApplicationUser() { UserName = model.UserName, Email = model.Email != null ? model.Email : "", UserId = mendeoUser.Entity.UserID };
        ...
        private IHttpActionResult GetErrorResultFromTuple(IList<Tuple<string, string>> result)
        {
            if (result == null)
            {
                return InternalServerError();
